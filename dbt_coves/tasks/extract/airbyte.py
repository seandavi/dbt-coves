...
from dbt_coves.utils.airbyte import AirbyteAPI
from dbt_coves.config.config import ExtractAirbyteModel
...
class AirbyteExtractorException(Exception):
    """Exception when Airbyte extractor fails"""
    pass


def run(
    config: ExtractAirbyteModel,
):
    ...
    # We should resolve these with CLI
    extract_destination_string = config.path
    airbyte_host = config.host
    airbyte_port = config.port
    dbt_modifiers = config.dbt_list_args
    ...
    extract_destination = Path(extract_destination_string)
    ...
    connections_extract_destination = connections_path.absolute()
    destinations_extract_destination = destinations_path.absolute()
    sources_extract_destination = sources_path.absolute()

    airbyte_api = AirbyteAPI(airbyte_host, airbyte_port)
    ...
    source_cmd = ["dbt", "ls", "--resource-type", "source", dbt_modifiers]

    # WTF, why are we assuming dbt dir
    if not dbt_packages_exist(os.getcwd()):
        raise AirbyteExtractorException(
            f"Could not locate dbt_packages folder, make sure 'dbt deps' was ran."
        )

    try:
        dbt_sources_list = subprocess.run(source_cmd, text=True, check=True)
    except subprocess.CalledProcessError as exc:
        raise AirbyteExtractorException(
            f"Command '{' '.join(source_cmd)}' failed and returned with error (code {exc.returncode})\n"
            f"Output: {exc.output.decode('utf-8')}"
        ) from exc

    if not dbt_sources_list:
        raise AirbyteExtractorException(
            f"No compiled dbt sources found running '{' '.join(source_cmd)}'"
        )

    manifest_json = json.load(open(Path() / "target" / "manifest.json"))

    dbt_sources_list = _clean_sources_prefixes(dbt_sources_list)
    ...
    for source in dbt_sources_list:
        ...
        source_connection = _get_airbyte_connection_for_table(
            source_db, source_schema, source_table
        )
        if source_connection:
            source_destination = _get_airbyte_destination_from_id(
                source_connection["destinationId"]
            )
            source_source = _get_airbyte_source_from_id(
    ...
    for conn in self.airbyte_api_caller.airbyte_connections_list:
        for stream in conn["syncCatalog"]["streams"]:
            if stream["stream"]["name"].lower() == table:
                ...
                if (
                    db
                    == destination_config["connectionConfiguration"]["database"].lower()
                ):
